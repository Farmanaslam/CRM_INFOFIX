...
import { User as AppUser, Task, Report, AppSettings, View } from "../../types";
import {
  ResponsiveContainer,
  BarChart,
  Bar,
...
export default function StaffReportsDashboard({
  ...
  const stats = useMemo(() => {
    return techs
      .map((tech) => {
        ...
        const techReports = savedReports.filter(
          (r) =>
  ...
  const teamTotals = useMemo(() => {
    const totalCompletedTasks = tasks.filter((t) => {
      ...
      const matchesZone =
        selectedZoneId === "all" || assignee?.zoneId === selectedZoneId;
    ...
    const totalFixedUnits = savedReports.filter((r) => {
      ...
      const matchesZone =
        selectedZoneId === "all" || tech?.zoneId === selectedZoneId;
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">
                Real-time Performance Governance
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 relative z-10">
          <div className="flex -space-x-3">
            {techs.slice(0, 4).map((t, i) => (
              <div
                key={i}
                className="w-10 h-10 rounded-xl border-2 border-white bg-slate-100 flex items-center justify-center text-xs font-black text-slate-400 overflow-hidden shadow-sm"
              >
                {t.photo ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            val: teamTotals.units,
            sub: "QC Verified Assets",
            icon: ShieldCheck,
            color: "blue",
          },
          {
            label: "Peak Performer",
            val: teamTotals.topPerformer,
            sub: "Alpha Performance",
            icon: Trophy,
            color: "amber",
          },
        ].map((kpi, i) => (
          <div
            key={i}
            className="bg-white border border-slate-200 rounded-[2.5rem] p-8 group hover:border-indigo-400 transition-all shadow-sm relative overflow-hidden"
          >
            <div
              className={`absolute -right-6 -top-6 p-6 opacity-[0.03] group-hover:scale-125 transition-transform duration-700 text-${kpi.color}-600`}
            >
              <kpi.icon size={100} />
            </div>
            <div className="flex justify-between items-start mb-4 relative z-10">
              <div
                className={`p-3 rounded-2xl bg-${kpi.color}-50 text-${kpi.color}-600 shadow-sm`}
              >
                <kpi.icon size={24} />
              </div>
              <div className="text-right">
                <span
                  className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-${kpi.color}-50 text-${kpi.color}-600`}
                >
                  Verified
                </span>
              </div>
            ...
            <h3
              className="text-2xl font-black text-slate-800 truncate mb-1 relative z-10"
            ...
            <div className="mt-4 flex items-center gap-2 relative z-10">
              <div
                className={`w-1.5 h-1.5 rounded-full bg-${kpi.color}-500 animate-pulse`}
              ></div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6 relative z-10">
            <div>
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter flex items-center gap-3">
                <Activity size={24} className="text-indigo-600" /> Output
                Volumetrics
              </h3>
              <p className="text-xs text-slate-400 mt-1 font-bold uppercase tracking-widest">
                Aggregate Technician Activity
              </p>
            </div>
        ...
        <div className="lg:col-span-4 bg-slate-900 p-10 rounded-[3rem] text-white shadow-xl flex flex-col relative overflow-hidden group">
          ...
          <div className="flex-1 h-64 relative z-10">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-4xl font-black text-white leading-none">
                {teamTotals.tasks + teamTotals.units}
              </span>
          ...
          <div className="space-y-3 mt-10 relative z-10">
            {teamMetrics.map((item, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 group/row hover:bg-white/10 transition-colors"
              >
                <div className="flex items-center gap-3">
                  ...
                  ></div>
                  <span className="text-xs font-black text-slate-400 uppercase tracking-widest group-hover/row:text-white transition-colors">
                    {item.name}
                  </span>
                </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-2">
                Historical Efficiency Ranking
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            <div className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 shadow-sm">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {stats.map((row, idx) => (
                <tr
                  key={idx}
                  className="hover:bg-slate-50/80 transition-all group duration-500"
                >
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-5">
                      <div className="relative">
                        <div
                          className={`w-14 h-14 rounded-[1.5rem] bg-white overflow-hidden border-2 transition-all group-hover:scale-110 duration-500 flex items-center justify-center font-black text-lg ${
                            idx === 0
                        ...
                        {idx < 3 && (
                          <div
                            className={`absolute -top-2 -right-2 w-8 h-8 rounded-full flex items-center justify-center border-4 border-white text-xs font-black text-white shadow-xl ${
                              idx === 0
                      ...
                      <div>
                        <p className="text-base font-black text-slate-800 leading-none mb-1.5 group-hover:text-indigo-600 transition-colors">
                          {row.name}
                        </p>
                        <div className="flex items-center gap-2">
                  ...
                  <td className="px-10 py-8 text-center">
                    <div className="inline-flex flex-col items-center">
                      <span className="text-xl font-black text-slate-700 leading-none mb-1.5">
                        {row.tasks}
                      </span>
                  ...
                  <td className="px-10 py-8">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        {row.efficiency}% Load Capacity
                      </span>
                    </div>
                  </td>
                  <td className="px-10 py-8 text-right">
                    <div
                      className={`inline-flex items-center gap-2 font-black text-[11px] uppercase tracking-tighter ${
                        idx === 0 ? "text-amber-500" : "text-emerald-500"
                      }`}
                    >
                      <TrendingUp size={16} /> +{10 + (stats.length - idx) * 5}%
                      Momentum
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={5} className="px-10 py-32 text-center">
                    ...
                    <p className="text-slate-300 text-sm mt-1">
                      No operational metrics captured for current cycle.
                    </p>
                  </td>
                </tr>
              )}
        ...
        <div className="px-10 py-6 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed">
              Analytics based on verified task completion and system-linked QC
              passports.
            </p>
          </div>
          <button
            onClick={() => onNavigate("staff_reports_financial")}
            className="flex items-center gap-2 text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:text-black transition-colors group"
          >
            Full Audit History{" "}
            <ArrowRight
              size={14}
