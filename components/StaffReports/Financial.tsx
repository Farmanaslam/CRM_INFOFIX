    ...
    const fetchLogs = async () => {
        ...
        setFinancialRecords(
            ...
            month: r.month,
            year: r.year,
            customersHandled: r.customers_handled,
            servicesDone: r.services_done,
    ...
    const accessibleTechs = useMemo(() => {
        ...
        case "ADMIN":
            return filtered.filter((m) => m.role !== "SUPER_ADMIN");

        case "MANAGER":
            return filtered.filter((m) => m.role === "TECHNICIAN");

        default: // TECHNICIAN
            return filtered.filter((m) => m.id === currentUser.id);
        }
    }, [teamMembers, selectedZoneId, currentUser]);
    ...
    const financialLogs = useMemo(() => {
        ...
        .filter((r) => {
            ...
            const matchesStaff =
            selectedStaffId === "all" || r.techId === selectedStaffId;
            const matchesZone =
            selectedZoneId === "all" ||
            tech?.zoneId === selectedZoneId ||
            !r.zoneId;

            const matchesRole =
            selectedRole === "all" || tech?.role === selectedRole;
        ...
        .map((record) => {
            const tech = teamMembers.find((m) => m.id === record.techId) || {
            name: "Unknown",
            photo: "",
    ...
    const kpis = useMemo(() => {
        ...
        const avgMargin =
        financialLogs.length > 0 ? (totalNetProfit / (totalSales || 1)) * 100 : 0;
    ...
    const staffHistory = useMemo(() => {
        ...
        .slice(0, 10);
    }, [formData.techId, financialRecords]);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        year: formData.year,
        customers_handled: formData.customersHandled,
        services_done: formData.servicesDone,
        sell_amount: formData.sellAmount,
        profit_amount: formData.profitAmount,
        ...
        ({ error } = await supabase.from("financial_logs").insert(payload));
        }
    ...
    const handleDelete = async (id: string) => {
        if (
        !window.confirm("Permanent deletion of this financial record. Proceed?")
        )
        return;
    ...
    return (
        ...
        <div className="bg-white p-5 lg:p-6 rounded-[2rem] lg:rounded-[3rem] border border-slate-200 shadow-sm flex flex-col xl:flex-row items-center justify-between gap-6">
            ...
            <div className="flex gap-1.5 shrink-0">
                ...
                className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-[9px] font-black uppercase text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 transition-all shadow-sm active:scale-95"
                >
                Month
                </button>
                <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div
                key={i}
                className="bg-white p-6 lg:p-8 rounded-[2rem] lg:rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden group"
            >
                <div
                className={`absolute -right-6 -top-6 p-6 opacity-[0.03] group-hover:scale-110 transition-transform duration-700 text-${card.color}-600`}
                >
                <card.icon size={120} />
                </div>
                <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mb-1 relative z-10">
                {card.label}
                </p>
                ...
                <span
                    className={`w-1.5 h-1.5 rounded-full bg-${card.color}-500 animate-pulse`}
                ></span>
        ...
        <div className="bg-white rounded-[2rem] lg:rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
            ...
            <div className="flex gap-2 bg-slate-100 p-1.5 rounded-2xl border border-slate-200 w-full lg:w-auto overflow-x-auto no-scrollbar">
                ...
                className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex-1 lg:flex-none ${
                    activeTab === "ledger"
                ...
                <tbody className="divide-y divide-slate-50">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3.5">
                            ...
                            {log.tech.photo ? (
                                ...
                                log.tech.name.charAt(0)
                            )}
                            </div>
                            <div className="min-w-0">
                            <p className="font-black text-slate-800 text-[13px] leading-none mb-1 truncate">
                                {log.tech.name}
                            </p>
                        ...
                        <td className="px-6 py-5">
                        <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border shadow-sm ${
                        ...
                        <div className="flex flex-col items-center gap-1">
                            ...
                            <div
                                className="px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded text-[9px] font-black uppercase border border-blue-100 whitespace-nowrap"
                                title="Jobs Done"
                            >
                                {log.servicesDone} J
                            </div>
                            <div
                                className="px-1.5 py-0.5 bg-purple-50 text-purple-600 rounded text-[9px] font-black uppercase border border-purple-100 whitespace-nowrap"
                                title="Customers Handled"
                            >
                                {log.customersHandled} C
                            </div>
                            </div>
                        </div>
                        </td>
                        ...
                        <div className="space-y-1">
                            ...
                            <span className="text-emerald-600">
                                ₹{log.profitAmount?.toLocaleString()}
                            </span>
                            </div>
                            <div className="flex justify-between w-36 text-[9px] font-bold">
                            ...
                            <span className="text-rose-400">
                                ₹
                                {(
                                (log.salaryAmount || 0) + (log.otherExpenses || 0)
                                ).toLocaleString()}
                            </span>
                            </div>
                        ...
                        <div className="flex flex-col items-center gap-1">
                            ...
                            <div
                                className={`h-full ${
                                log.margin > 30 ? "bg-indigo-500" : "bg-slate-400"
                                }`}
                                style={{
                                width: `${Math.min(
                            ...
                            {log.margin.toFixed(0)}% Profitability
                            </span>
                        </div>
                        </td>
                        ...
                        {log.bonus > 0 ? (
                            ...
                            <span className="text-[8px] font-bold text-indigo-400 uppercase tracking-widest mt-1.5 italic whitespace-nowrap">
                                5% Direct Share
                            </span>
                            </div>
                        ) : (
                        ...
                        <div className="flex gap-1.5 justify-end opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-0 translate-x-2">
                            ...
                            className="p-2 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-rose-500 hover:border-rose-200 shadow-sm transition-all"
                            >
                            <Trash2 size={12} />
                            </button>
                        </div>
                    ...
                    <tr>
                        ...
                        <p className="text-slate-300 text-xs mt-1">
                            Select a different range, role or staff member.
                        </p>
                        </td>
                    </tr>
                    )}
                ...
                <tbody className="divide-y divide-slate-50">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {agg.tech.photo ? (
                                ...
                                agg.tech.name.charAt(0)
                            )}
                            </div>
                            <div className="min-w-0">
                            <p className="font-black text-slate-800 text-sm tracking-tight leading-none mb-1.5 truncate">
                                {agg.tech.name}
                            </p>
                            <div className="flex items-center gap-2">
                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-1.5 py-0.5 rounded">
                                {agg.tech.role.replace("_", " ")}
                                </span>
                                <span className="text-[8px] font-bold text-indigo-500 uppercase tracking-widest whitespace-nowrap">
                                Active Specialist
                                </span>
                            </div>
                            </div>
                        ...
                        <div className="inline-flex flex-col items-center gap-1">
                            ...
                            {agg.totalServices} J / {agg.totalCustomers} C
                            </span>
                            <div className="w-16 h-1 bg-slate-100 rounded-full overflow-hidden">
                            <div
                                className="h-full bg-indigo-500"
                                style={{
                                width: `${Math.min(agg.totalServices * 2, 100)}%`,
                                }}
                            ></div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            ₹{agg.totalProfit.toLocaleString()}
                            </p>
                        </div>
                        </td>
                        <td className="px-6 py-6 text-center">
                        <div
                            className={`inline-flex flex-col items-center px-4 py-1.5 rounded-xl border ${
                            agg.totalNet >= 0
                    ...
                    <tr>
                        ...
                        <h4 className="text-lg font-black text-slate-300 uppercase tracking-widest">
                            No Member Data Captured
                        </h4>
                        <p className="text-slate-400 text-sm mt-1">
                            System is awaiting performance logs for the selected
                            criteria.
                        </p>
                        </td>
                    </tr>
        ...
        {showAddModal && (
            ...
            <div className="bg-white rounded-none sm:rounded-[3rem] w-full max-w-6xl shadow-2xl animate-in fade-in zoom-in-95 duration-500 overflow-hidden flex flex-col h-full sm:max-h-[95vh] border border-white/20">
                ...
                <div className="flex items-center gap-4 lg:gap-5">
                    <div
                    className={`w-10 h-10 lg:w-12 lg:h-12 rounded-xl flex items-center justify-center text-white shadow-xl ${
                        editingRecordId
                    ...
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">
                        Asset & Productivity Ledger
                    </p>
                    </div>
                </div>
                <button
                    onClick={() => setShowAddModal(false)}
                    className="p-2 lg:p-2.5 hover:bg-slate-100 rounded-full text-slate-400 transition-all active:scale-90"
                >
                    <X size={24} />
                </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 lg:p-10 bg-slate-50/30">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                onChange={(e) =>
                                    setFormData({
                                    ...formData,
                                    servicesDone: parseInt(e.target.value) || 0,
                                    })
                                }
                                className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-lg lg:text-xl font-black text-slate-800 outline-none focus:bg-white transition-colors"
                                />
                        ...
                        <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm grid grid-cols-2 gap-x-4 gap-y-4">
                            ...
                            <input
                                ...
                                setFormData({
                                    ...formData,
                                    sellAmount: parseFloat(e.target.value) || 0,
                                })
                                }
                            ...
                            <input
                                ...
                                setFormData({
                                    ...formData,
                                    profitAmount: parseFloat(e.target.value) || 0,
                                })
                                }
                    ...
                    <div className="bg-slate-900 p-6 lg:p-8 rounded-[2rem] lg:rounded-[2.5rem] text-white relative overflow-hidden shadow-2xl">
                        ...
                        <div className="flex items-center gap-5">
                            ...
                            <h5 className="text-2xl lg:text-3xl font-black text-white whitespace-nowrap">
                                ₹{projection.net.toLocaleString()}{" "}
                                <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider ml-1">
                                Net Flow
                                </span>
                            </h5>
                            </div>
                        ...
                        <div className="flex flex-col items-center md:items-end w-full md:w-auto">
                            ...
                            Automated Profit Share
                            </p>
                            <div
                            className={`px-6 lg:px-8 py-3 lg:py-4 rounded-2xl lg:rounded-[1.75rem] text-xl lg:text-2xl font-black transition-all duration-500 w-full md:w-auto text-center ${
                                projection.bonus > 0
                                ? "bg-indigo-600 shadow-xl shadow-indigo-500/30 scale-105 ring-4 ring-indigo-400/20"
                    ...
                    <div className="flex gap-4 pt-4 shrink-0">
                        ...
                        type="button"
                        onClick={() => setShowAddModal(false)}
                        className="flex-1 py-4 bg-white border border-slate-200 text-slate-400 font-black rounded-2xl lg:rounded-[2rem] text-[10px] uppercase tracking-widest hover:bg-slate-100 transition-all active:scale-95"
                        >
                        Discard Entry
                        </button>
                        <button
                        type="submit"
                        disabled={!formData.techId}
                        className="flex-[2.5] py-4 bg-indigo-600 text-white font-black rounded-2xl lg:rounded-[2rem] text-[10px] lg:text-[11px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-50"
                        >
                        <Save size={18} />{" "}
                        {editingRecordId ? "Apply Correction" : "Commit Registry"}
                        </button>
                    </div>
                    </form>
                </div>
                ...
                <div className="hidden lg:block lg:w-1/3 bg-slate-100 p-8 border-l border-slate-200 overflow-y-auto custom-scrollbar">
                    ...
                    {staffHistory.map((entry) => (
                        <div
                        key={entry.id}
                        className="bg-white p-4 lg:p-5 rounded-[2rem] border border-slate-200 shadow-sm space-y-4 group hover:border-indigo-400 transition-colors"
                        >
                        <div className="flex justify-between items-start">
                            <div>
                            <p className="text-[10px] font-black text-slate-800 uppercase tracking-tight">
                                {entry.day} {months[entry.month]}
                            </p>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-center border-t border-slate-50 pt-3">
                            ...
                            <p className="text-[11px] font-bold text-slate-700">
                                ₹{entry.sellAmount?.toLocaleString()}
                            </p>
                            </div>
                            <div>
                            <p className="text-[8px] font-black text-slate-400 uppercase mb-0.5">
