    ...
    <>
      ...
      */}
      ...
      `}>
        ...
        {/* Brand Header */}
        <div className={`flex items-center gap-4 px-7 py-10 shrink-0 ${!showLabels ? 'justify-center px-4' : ''}`}>
          <div className="relative group cursor-pointer" onClick={() => setIsCollapsed(false)}>
             <div className={`w-10 h-10 lg:w-12 lg:h-12 bg-gradient-to-tr ${isSuperAdmin ? 'from-purple-600 to-indigo-500' : 'from-indigo-600 to-indigo-400'} rounded-2xl flex items-center justify-center text-white shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-transform hover:scale-105 active:scale-95`}>
          ...
          <div className={`transition-opacity duration-300 ${showLabels ? 'opacity-100' : 'opacity-0 hidden w-0'}`}>
             ...
             <p className="text-[9px] font-black text-indigo-400/80 uppercase tracking-[0.2em] mt-1.5 flex items-center gap-1.5">
                <span className="w-1 h-1 rounded-full bg-indigo-500"></span> {isSuperAdmin ? 'Services' : 'System'}
             </p>
          </div>
        </div>
        ...
        {currentUser.role !== 'CUSTOMER' && (
            <div className={`px-6 mb-4 ${!showLabels ? 'px-3' : ''}`}>
                {isSuperAdmin ? (
                    <div className={`bg-white/5 border border-white/10 rounded-2xl p-3 backdrop-blur-md group hover:border-indigo-500/50 transition-all relative overflow-hidden ${!showLabels ? 'flex justify-center cursor-pointer' : ''}`} onClick={() => !showLabels && setIsCollapsed(false)}>
                        <MapPin size={16} className={`text-indigo-400 ${!showLabels ? '' : 'absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none'}`} />
                        {showLabels && (
                          <>
                            <select
                            ...
                            >
                                <option value="all" className="bg-[#0f172a] text-white">All Zones</option>
                    ...
                    userZone && (
                        <div className={`bg-white/5 border border-white/10 rounded-2xl p-3 flex items-center gap-3 backdrop-blur-md ${!showLabels ? 'justify-center' : ''}`} title={userZone.name}>
                            <div className="w-8 h-8 rounded-lg bg-indigo-500/20 flex items-center justify-center text-indigo-400 shrink-0">
                                <MapPin size={16} />
                            </div>
                            {showLabels && (
                              <div className="overflow-hidden">
                                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest truncate">Active Zone</p>
                                  <p className="text-xs font-black text-white truncate">{userZone.name}</p>
                              </div>
                            )}
                        </div>
                    )
          ...
          {allNavItems.map((item) => {
            ...
            return (
              <div key={item.id} className="relative group/nav">
                {isActive && (
                  <div className={`absolute top-1/2 -translate-y-1/2 w-1.5 h-6 bg-indigo-500 rounded-r-full shadow-[0_0_10px_rgba(99,102,241,0.5)] ${!showLabels ? 'left-[-12px]' : 'left-[-16px]'}`}></div>
                )}
                ...
                <button
                  ...
                  className={`
                    ...
                    ${isActive ? 'bg-white/[0.06] text-white' : 'hover:bg-white/[0.03] hover:text-slate-100 text-slate-500'}
                    ${!showLabels ? 'justify-center' : 'justify-between'}
                  `}
                  title={!showLabels ? item.label : undefined}
                >
                  <div className="flex items-center gap-3.5">
                    <Icon size={20} className={isActive ? 'text-indigo-400 shadow-[0_0_8px_rgba(129,140,248,0.4)]' : 'text-slate-600'} />
                    {showLabels && <span className="tracking-tight">{item.label}</span>}
                  </div>
                    ...
                    <div className="bg-slate-900 border border-white/10 rounded-2xl shadow-2xl p-2 min-w-[200px]">
                      ...
                      {visibleChildren.map(child => (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[12px] font-bold transition-all ${currentView === child.id ? 'text-white bg-indigo-600' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                        >
                          <child.icon size={14} />
                          {child.label}
                        </button>
        ...
        <div className="p-4 shrink-0">
          {showLabels ? <InstallPWA /> : (
             <div className="flex justify-center mb-2">
                <div className="p-2 bg-white/5 rounded-lg text-slate-500 hover:text-white cursor-pointer" title="Install App">
                   <Zap size={16} />
                </div>
             </div>
          )}
          
          <div className={`bg-white/[0.03] backdrop-blur-2xl rounded-2xl border border-white/[0.05] shadow-2xl transition-all ${showLabels ? 'p-4' : 'p-2 flex justify-center'}`}>
             <div className={`flex items-center ${showLabels ? 'justify-between' : 'justify-center flex-col gap-3'}`}>
