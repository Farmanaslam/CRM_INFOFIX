...
interface SystemLearnings {
  commonIssues: { issue: string; count: number }[];
  busyTechs: { name: string; count: number }[];
  vipCustomers: { name: string; ticketCount: number }[];
  qcFailures: { rate: number; count: number };
}
...
export default function Supports({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "System initialized. Cognitive Engine V3.5-PRO online.\n\nI have indexed all active service nodes, customer profiles, workforce schedules, and QC reports. How may I facilitate your operations today?",
      timestamp: new Date(),
    },
  ]);
  ...
  // This memoized object aggregates raw data into usable intelligence
  const systemKnowledge: SystemLearnings = useMemo(() => {
    // 1. Ticket Patterns
    const issueMap: Record<string, number> = {};
    tickets.forEach((t) => {
      const words = t.issueDescription
        .toLowerCase()
        .split(" ")
        .filter((w) => w.length > 4);
      words.forEach((w) => (issueMap[w] = (issueMap[w] || 0) + 1));
    });
    ...
    tasks
      .filter((t) => t.status === "pending" && t.assignedToId)
      .forEach((t) => {
        const techName =
          settings.teamMembers.find((m) => m.id === t.assignedToId)?.name ||
          "Unknown";
        techMap[techName] = (techMap[techName] || 0) + 1;
      });
    const busyTechs = Object.entries(techMap)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
    ...
    tickets.forEach((t) => {
      if (t.name) custMap[t.name] = (custMap[t.name] || 0) + 1;
    });
    const vipCustomers = Object.entries(custMap)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(([name, ticketCount]) => ({ name, ticketCount }));

    // 4. QC Intelligence
    const failedQC = reports.filter(
      (r) => r.progress < 100 || r.actionRequired
    ).length;
    const qcFailures = {
      count: failedQC,
      rate:
        reports.length > 0 ? Math.round((failedQC / reports.length) * 100) : 0,
    };

    return { commonIssues, busyTechs, vipCustomers, qcFailures };
  }, [tickets, tasks, customers, reports, settings.teamMembers]);

  // Insights derived for the sidebar
  const insights = useMemo(() => {
    return [
      {
        id: "p1",
        title: "Top Issues",
        count: systemKnowledge.commonIssues.reduce((a, b) => a + b.count, 0),
        description: `Frequent terms: ${systemKnowledge.commonIssues
          .map((i) => i.issue)
          .join(", ")}`,
        relatedTickets: [],
      },
      {
        ...
        description: `Most active: ${systemKnowledge.busyTechs
          .map((t) => t.name)
          .join(", ")}`,
        relatedTickets: [],
      },
      {
        id: "p3",
        title: "VIP Clients",
        count: systemKnowledge.vipCustomers.length,
        description: `Top clients: ${systemKnowledge.vipCustomers
          .map((c) => c.name)
          .join(", ")}`,
        relatedTickets: [],
      },
      {
        id: "p4",
        title: "QC Flags",
        count: systemKnowledge.qcFailures.count,
        description: `${systemKnowledge.qcFailures.rate}% of recent devices required rework.`,
        relatedTickets: [],
      },
    ];
  }, [systemKnowledge, tasks]);

  const cognitiveLoad = useMemo(() => {
    const dataPoints =
      tickets.length + customers.length + tasks.length + reports.length;
    return Math.min(100, Math.round((dataPoints / 1000) * 100)); // Arbitrary scale for UI
  }, [tickets, tasks, customers, reports]);
  ...
  const handleSendMessage = async (
    ...
    forcedType: "text" | "draft" | "diagnostic" = "text"
  ) => {
    ...
    const kbContext = settings.supportGuidelines
      .map(
        (g) => `[PROTOCOL: ${g.title}] CAT: ${g.category}\nINFO: ${g.content}`
      )
      .join("\n\n");

    // Inferred patterns from actual data
    const inferredPatterns = `
      [INFERRED PATTERNS FROM LIVE DATABASE]:
      - Common Fault Keywords: ${systemKnowledge.commonIssues
        .map((i) => i.issue)
        .join(", ")}.
      - Busiest Technicians: ${systemKnowledge.busyTechs
        .map((t) => `${t.name} (${t.count} tasks)`)
        .join(", ")}.
    ...
    const systemPrompt = `
      ...
      AUTO-LEARNED INTELLIGENCE (DYNAMIC):
      ${inferredPatterns}
      
      INSTRUCTIONS:
      1. You have access to the entire shop's database summary above (Auto-Learned Intelligence). Use it to answer questions about workload, common issues, or customers.
      2. If asked about a specific ticket (Session Focus), analyze it deeply.
      3. If requested to "Draft", use a premium, professional corporate tone.
      4. If "Diagnose", provide step-by-step technical isolation procedures.
      5. Reference internal Protocols by name if they match the issue.
      6. Always present data in a clean, structured pipeline format.
      7. Current Response Target: ${forcedType.toUpperCase()}.
    `;
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            "CRITICAL: Neural link interrupted. Re-establish sync to continue.",
          timestamp: new Date(),
        },
      ]);
    } finally {
  ...
  const handleDeleteGuideline = (id: string) => {
    if (
      confirm("Are you sure you want to delete this protocol from the vault?")
    ) {
      const updated = settings.supportGuidelines.filter(
        (item) => item.id !== id
      );
  ...
  const handleFormalizePattern = (pattern: any) => {
    setEditingGuideline({
      id: Date.now().toString(),
      title: `${pattern.title} Analysis`,
      category: "Diagnostic",
      content: `System detected pattern: ${pattern.title}.\n\nContext: ${pattern.description}\n\nRecommended Action:\n1. Verify if incoming devices match this profile.\n2. Apply standard fixes for this category.`,
    });
      ...
      <div className="flex-1 flex flex-col bg-white rounded-[2.5rem] lg:rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden relative animate-in fade-in duration-1000 h-full min-w-0">
          ...
          <div className="flex items-center gap-4">
            {!showSidebar && (
              <button
                ...
                className="p-2.5 bg-slate-900 text-indigo-400 hover:text-white rounded-xl transition-all shadow-lg mr-1"
              >
                <Terminal size={18} />
              </button>
            )}
            ...
            <div>
              <h2 className="font-black text-slate-800 text-sm lg:text-xl uppercase tracking-tighter leading-none flex items-center gap-2">
                COGNITIVE ENGINE
                <span className="text-[8px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg border border-indigo-100 shadow-sm ml-1">
                  V3.5-PRO
                </span>
              </h2>
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <span className="hidden lg:inline">
                {showKnowledgeBase ? "CLOSE VAULT" : "OPEN VAULT"}
              </span>
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 lg:p-8 space-y-8 bg-slate-50/30 custom-scrollbar min-h-0 relative">
          <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-[0.02]">
            <Waves
              size={800}
              className="text-slate-900 absolute -top-40 -left-40"
            />
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-4 mb-2 no-scrollbar">
            {quickActions.map((action, idx) => (
              <button
                key={idx}
                onClick={() =>
                  handleSendMessage(action.prompt, action.type as any)
                }
          ...
          <div className="relative flex items-center gap-3">
            <div className="flex-1 relative group">
              ...
              <input
                ...
                placeholder={
                  activeTicket
                    ? `Prompt for ${activeTicket.ticketId}...`
                    : "Command the global engine..."
                }
            ...
            {kbTab === "insights" ? (
              <div className="space-y-4">
                <div className="p-5 bg-slate-900 rounded-[2rem] text-white relative overflow-hidden group shadow-xl">
                  ...
                  <p className="text-[11px] font-medium leading-relaxed text-indigo-100/90 italic">
                    "Autonomous pattern detection is isolating common fault
                    clusters from real-time operations."
                  </p>
                </div>
                {insights.map((pattern) => (
                  <div
                    key={pattern.id}
                    className="p-5 bg-white border border-slate-100 rounded-[2rem] hover:border-indigo-400 transition-all group shadow-sm hover:shadow-lg"
                  >
                    <div className="flex justify-between items-start mb-3">
                      <h4 className="font-black text-slate-800 text-[11px] uppercase tracking-tight">
                        {pattern.title}
                      </h4>
                    ...
                    <p className="text-[10px] text-slate-500 font-medium mb-4 leading-relaxed italic line-clamp-3">
                      "{pattern.description}"
                    </p>
                    <button
              ...
              <div className="space-y-4">
                {filteredKB.map((g) => (
                  <div
                    key={g.id}
                    className="p-5 bg-white border border-slate-100 rounded-[2rem] hover:border-indigo-400 transition-all group relative shadow-sm hover:shadow-lg"
                  >
                    <div className="flex justify-between items-start mb-3">
                    ...
                    <h4 className="font-black text-slate-800 text-[11px] mb-2 leading-tight uppercase tracking-tight">
                      {g.title}
                    </h4>
                    <p className="text-[10px] text-slate-500 leading-relaxed font-medium line-clamp-3 italic">
                      "{g.content}"
                    </p>
                  </div>
                ))}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-white rounded-[2.5rem] w-full max-w-4xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-500 flex flex-col max-h-[90vh] border border-white/20">
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-12 opacity-[0.03] pointer-events-none text-slate-900 group-hover:rotate-12 transition-transform duration-1000">
                <BrainCircuit size={160} />
              </div>
              <div className="flex items-center gap-5 relative z-10">
                <div className="w-12 h-12 bg-indigo-600 text-white rounded-2xl shadow-xl flex items-center justify-center">
                  <BookOpen size={24} />
                </div>
                <div>
                  <h3 className="font-black text-slate-800 text-xl lg:text-2xl uppercase tracking-tighter leading-none">
                    {editingGuideline ? "Adjust Node" : "Neural Induction"}
                  </h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-2">
                    Intelligence Allocation Layer
                  </p>
                </div>
              </div>
              <button
                onClick={() => setIsGuidelineModalOpen(false)}
                className="p-3 hover:bg-white rounded-full text-slate-400 transition-all border border-transparent active:scale-90"
              >
                <X size={28} />
              </button>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3">
                    Instructional Payload (Protocol Details)
                  </label>
            ...
            <div className="px-8 py-6 border-t border-slate-100 bg-white flex flex-col sm:flex-row gap-4 shrink-0">
              <button
                onClick={() => setIsGuidelineModalOpen(false)}
                className="flex-1 py-4 bg-slate-50 text-slate-500 font-black rounded-2xl text-[10px] uppercase tracking-widest hover:bg-slate-100 transition-all active:scale-95"
              >
                Discard
              </button>
              <button
