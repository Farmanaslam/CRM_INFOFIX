...
const NOTIFICATION_SOUND = "data:audio/mp3;base64,";

export default function NotificationHub({ notifications, setNotifications, isOpen, onClose, onNavigate, currentUser }: NotificationHubProps) {
  const [activeFilter, setActiveFilter] = useState<'all' | 'urgent' | 'system'>('all');
  ...
  const filteredNotifications = useMemo(() => {
    ...
    let result = notifications.filter(n => {
      ...
      if (currentUser.role === 'MANAGER') {
        ...
        if (n.userRole === 'SUPER_ADMIN' || n.userRole === 'ADMIN') return false; 
      ...
      // 2. Type Filtering
      if (activeFilter === 'urgent' && (n.type !== 'urgent' && n.type !== 'warning')) return false;
      if (activeFilter === 'system' && (n.type !== 'info' && n.type !== 'success')) return false;

      // 3. Admin User Role Filtering
      if (isAdmin && userRoleFilter !== 'ALL' && n.userRole !== userRoleFilter) return false;
    ...
    return result;
  }, [notifications, activeFilter, sortOrder, isAdmin, currentUser, userRoleFilter]);
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'urgent': return <Flame size={18} className="text-white" />;
  ...
  const getColor = (type: string) => {
    switch (type) {
      case 'urgent': return 'bg-rose-500 shadow-rose-200';
      ...
      <div className="relative w-full max-w-md h-full bg-white shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
        ...
        <div className="px-5 py-3 bg-slate-50 border-b border-slate-200 flex flex-col gap-3">
           <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                 <button 
                    ...
                    className={`p-2 rounded-lg border transition-colors ${soundEnabled ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-slate-200 text-slate-400'}`}
                    title="Toggle Sound"
                 >
           ...
           <div className="flex justify-between items-center text-xs pt-1">
              <span className="font-bold text-slate-500">{filteredNotifications.length} Messages</span>
              <div className="flex gap-3">
                 <button onClick={markAllRead} className="text-indigo-600 hover:text-indigo-700 font-bold flex items-center gap-1">
                    <CheckCheck size={14} /> Mark Read
                 </button>
                 <button onClick={clearAll} className="text-slate-400 hover:text-rose-500 font-bold flex items-center gap-1 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2 bg-slate-50/50">
           {filteredNotifications.length > 0 ? (
              filteredNotifications.map((note) => (
                 ...
                 >
                    ...
                    <div className="flex gap-4">
                       <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg shrink-0 ${getColor(note.type)}`}>
                          {getIcon(note.type)}
                       </div>
                       
                       <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start mb-1">
                             <h4 className={`text-sm font-bold truncate ${note.read ? 'text-slate-600' : 'text-slate-900'}`}>
                                {note.title}
                             </h4>
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-4">
                 ...
                 <p className="text-sm font-bold uppercase tracking-widest opacity-50">All caught up</p>
              </div>
           )}
        </div>
      </div>
