...
export default function LaptopReports({ activeTab, settings, currentUser, reports = [], setReports, selectedZoneId }: LaptopReportsProps) {
  ...
  const filteredReports = useMemo(() => {
    ...
    return zoneFilteredReports.filter(r => {
        ...
        return matchesSearch && matchesTech && matchesDealer && matchesAction && matchesStatus;
    });
  }, [zoneFilteredReports, searchTerm, filterTech, filterDealer, filterAction, filterStatus]);
  ...
  const handleSaveReport = () => {
    ...
    const reportToSave: Report = {
        ...
        // Fix: Ensure the report is tagged with the current active zone if it's a new report
        zoneId: currentReport.zoneId || (selectedZoneId !== 'all' ? selectedZoneId : undefined)
    };
  ...
  const handleExportFilteredPDF = () => {
    ...
    doc.text(`Filter View: Tech: ${filterTech}, Dealer: ${filterDealer}, Status: ${filterStatus}`, 15, 21);

    // Table Setup
    let y = 35;
    const headers = ["Device No", "Date", "Dealer", "Technician", "Progress", "Status", "Action Req."];
    ...
    filteredReports.forEach((report, index) => {
        if (y > 185) { doc.addPage('l'); y = 25; } // Page break
        ...
        currentX = startX;
        doc.text(report.deviceInfo.laptopNo, currentX + 2, y); currentX += colWidths[0];
        doc.text(new Date(report.date).toLocaleDateString(), currentX + 2, y); currentX += colWidths[1];
        doc.text(report.deviceInfo.customerName || 'N/A', currentX + 2, y, { maxWidth: colWidths[2] - 4 }); currentX += colWidths[2];
        doc.text(report.deviceInfo.technicianName || 'N/A', currentX + 2, y, { maxWidth: colWidths[3] - 4 }); currentX += colWidths[3];
        doc.text(`${report.progress}%`, currentX + 2, y); currentX += colWidths[4];
        doc.text(report.status, currentX + 2, y); currentX += colWidths[5];
        doc.text(report.actionRequired || 'None', currentX + 2, y, { maxWidth: colWidths[6] - 4 });
      ...
      <div className="space-y-6 h-full flex flex-col bg-slate-50 bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] [background-size:16px_16px]">
        ...
        <div className="bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-2xl shrink-0">
           ...
           <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
                 <h1 className="text-3xl font-black mb-2 tracking-tight text-white">Laptop QC Management</h1>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {filteredReports.length === 0 && (
                    <div className="col-span-full py-12 text-center text-slate-400">
                        ...
                        <button onClick={() => { setSearchTerm(''); setFilterTech('All'); setFilterDealer('All'); setFilterAction('All'); setFilterStatus('All'); }} className="text-indigo-600 text-xs font-bold mt-2 hover:underline">Clear all filters</button>
                    </div>
                )}
             </div>
           ) : (
