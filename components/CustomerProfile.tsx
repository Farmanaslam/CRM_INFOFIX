...
import { User as UserIcon, Mail, Phone, MapPin, Save, CheckCircle, Smartphone, Clock, Shield, Key, Camera, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function CustomerProfile({ currentUser, customers, setCustomers, updateCurrentUser, tickets }: CustomerProfileProps) {
  ...
  const activeTickets = myTickets.filter(t => t.status !== 'Resolved' && t.status !== 'Rejected').length;
  const joinDate = myTickets.length > 0 ? myTickets[myTickets.length - 1].date : new Date().toLocaleDateString(); 
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Update Customer Record (Customers array usually doesn't store photo, but User does)
    // We update the Customer record primarily for contact info
    if (customerRecord) {
        const updatedCustomers = customers.map(c =>
            c.id === customerRecord.id
            ? { ...c, name: formData.name, mobile: formData.mobile, address: formData.address }
            : c
        );
        setCustomers(updatedCustomers);
    }

    // Update Current Session User (This stores the photo)
    updateCurrentUser({
        ...currentUser,
        name: formData.name,
          ...
          <div className="absolute -bottom-16 left-8 flex items-end gap-6">
              <div className="relative group">
                <div className="w-32 h-32 rounded-full bg-white p-1.5 shadow-xl">
                    <div className="w-full h-full rounded-full bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200">
                        {formData.photo ? (
                          ...
                          <span className="text-slate-400 font-bold text-4xl">{formData.name.charAt(0)}</span>
                        )}
                    </div>
                </div>
                ...
                <button 
                  ...
                  title="Change Photo"
                >
                  {isCompressing ? <Loader2 size={16} className="animate-spin" /> : <Camera size={16} />}
                </button>
                <input 
                  type="file" 
                  ref={fileInputRef} 
              ...
              <div className="mb-4">
                  ...
                  <p className="text-slate-500 font-medium flex items-center gap-2">
                     <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wide">Customer</span>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="space-y-6">
               ...
               <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                   <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide mb-4">Account Overview</h3>
                   <div className="space-y-4">
                       <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                           <div className="flex items-center gap-3">
                               <div className="p-2 bg-blue-100 text-blue-600 rounded-lg"><Smartphone size={18}/></div>
                               <div>
                                   <p className="text-xs text-slate-500 font-bold uppercase">Total Repairs</p>
                       ...
                       <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                           <div className="flex items-center gap-3">
                               ...
                               <div>
                                   <p className="text-xs text-slate-500 font-bold uppercase">Active Service</p>
                       ...
                       <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                           <div className="flex items-center gap-3">
                               <div className="p-2 bg-emerald-100 text-emerald-600 rounded-lg"><Shield size={18}/></div>
                               <div>
                                   <p className="text-xs text-slate-500 font-bold uppercase">First Service</p>
                                   <p className="font-bold text-slate-800 text-sm">{joinDate.split(',')[0]}</p>
                               </div>
                           </div>
                       </div>
                   </div>
               ...
               {/* Login Info Card */}
               <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                   <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide mb-4">Login Credentials</h3>
                   <div className="space-y-4">
                       <div>
                           <label className="text-xs font-semibold text-slate-400 block mb-1">Email ID (Username)</label>
                       ...
                       <div>
                           ...
                           <div className="flex items-center gap-2 text-slate-700 font-medium bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                               ...
                               <span className="text-slate-500 text-sm italic">Your registered mobile number</span>
                           </div>
                       </div>
                   </div>
               </div>
               ...
               <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
                   ...
                   <form onSubmit={handleSave} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5 ml-1">Mobile Number</label>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                                        <Phone size={18} />
                                    </div>
                                ...
                                <p className="text-[10px] text-slate-400 mt-1 ml-1">Note: This will update your login password.</p>
                            </div>
                        </div>

                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5 ml-1">Shipping / Billing Address</label>
                            <div className="relative">
                        ...
                        {success && (
                            <div className="p-4 bg-emerald-50 text-emerald-700 text-sm font-bold rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2 border border-emerald-100">
                                <CheckCircle size={20} className="fill-current text-emerald-200" />
                        ...
                        <div className="pt-4 flex justify-end">
                            ...
                            >
                                <Save size={18} /> {isCompressing ? 'Processing...' : 'Save Changes'}
                            </button>
                        </div>
                   </form>
               </div>
