...
export default function Reports({ tickets, settings }: ReportsProps) {
  ...
  const analytics = useMemo(() => {
    ...
    resolvedTickets.forEach(t => {
       if (t.history && t.history.length > 0) {
          ...
          if (resolvedLog) {
             totalDays += calculateDaysDiff(t.date, resolvedLog.date.split(' ')[0]);
    ...
    filteredData.forEach(t => {
        if (t.assignedToId) {
            ...
            if (!techMap[techName]) techMap[techName] = { name: techName, count: 0, resolved: 0 };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`InfoFix_Work_Report_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (e) {
      console.error(e);
    } finally {
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sticky top-0 z-20 backdrop-blur-xl bg-white/90">
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h2 className="text-xl font-bold text-slate-800">Work Status Reports</h2>
                <p className="text-xs text-slate-500">Service operations & turnaround metrics</p>
             </div>
          </div>
      ...
      <div id="report-dashboard" className="space-y-6">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
               ...
               <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Total Intake</p>
               <h3 className="text-3xl font-black text-slate-800">{analytics.totalTickets} <span className="text-lg text-slate-400 font-medium">Tickets</span></h3>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
               ...
               <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Active Workload</p>
               <h3 className="text-3xl font-black text-slate-800">{analytics.openTickets} <span className="text-lg text-slate-400 font-medium">Open</span></h3>
               <div className="flex items-center gap-2 mt-4">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
               <div className="flex items-center justify-between mb-6">
                  <div>
                     <h3 className="font-bold text-slate-800 text-lg">Ticket Volume Trend</h3>
                     <p className="text-xs text-slate-500">Service intake counts over the selected period</p>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-medium">
                     <span className="flex items-center gap-1 text-indigo-600"><div className="w-2 h-2 bg-indigo-500 rounded-full"></div> New Jobs</span>
                  </div>
               </div>
                     ...
                     <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={analytics.trendData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                           ...
                           <Tooltip 
                              ...
                              cursor={{ fill: '#f1f5f9' }}
                           />
                           <Area type="monotone" dataKey="tickets" stroke={COLORS.primary} strokeWidth={4} fill="url(#colorTickets)" />
                        </AreaChart>
                     </ResponsiveContainer>
                  </div>
               </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
               <h3 className="font-bold text-slate-800 text-lg mb-2">Service Status</h3>
               <div className="relative w-full h-[250px]">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
                     ...
                     <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Reports</span>
                  </div>
               </div>
            </div>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
               <h3 className="font-bold text-slate-800 text-lg mb-6 flex items-center gap-2">
                  <Users size={20} className="text-indigo-600"/> Staff Performance (Volume)
               </h3>
                     ...
                     <div key={idx} className="flex items-center gap-4 p-3 hover:bg-slate-50 rounded-xl transition-colors border border-transparent hover:border-slate-100">
                        ...
                        <div className="flex-1">
                           <div className="flex justify-between mb-1">
                              ...
                              <span className="text-xs font-bold text-indigo-600">{tech.resolved} Resolved</span>
                           </div>
                           ...
                           <div className="flex justify-between mt-1 text-[10px] text-slate-400">
                              ...
                              <span>{Math.round((tech.resolved / tech.count) * 100)}% Success</span>
                           </div>
                        </div>
                     </div>
                  ))}
                  {analytics.techPerformance.length === 0 && (
                     <div className="text-center py-8 text-slate-400 text-sm">No technician activity recorded.</div>
                  )}
               </div>
            </div>
