...
export default function Schedule({
  ...
  const events = useMemo(() => {
    ...
    // ---- TICKETS (always visible based on backend rules) ----
    ...
    const visibleTasks = tasks.filter((t) => {
      ...
      if (currentUser.role === "MANAGER") {
        ...
        // Assigned to manager himself
        if (t.assignedToId === currentUser.id) return true;

        // Assigned to technician or another manager
        if (
          assignedUser &&
    ...
    return allEvents;
  }, [tickets, tasks, settings.teamMembers, currentUser]);
  ...
  const handleToggleTaskStatus = async (taskId: string) => {
    ...
    if (!error && data) {
      setTasks(tasks.map((t) => (t.id === taskId ? mapDbTask(data) : t)));
    }
  };
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          ...
          className={`h-24 sm:h-32 border border-slate-100 p-2 relative cursor-pointer transition-all group ${
            isSelected
      ...
      <div className="w-full lg:w-96 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-gradient-to-br from-indigo-600 to-violet-600 text-white">
          <h3 className="text-lg font-bold flex items-center gap-2">
            <CalendarIcon size={20} className="text-indigo-200" /> Daily Agenda
          </h3>
                          ...
                          <button
                            onClick={() => {
                              if (isAdmin) {
                                ...
                                handleEditTask({
                                  ...(evt.originalData as Task),
                                  title: evt.title, // lock fields
                                  date: evt.date,
                                });
                              }
                            }}
                            title={
                              isAdmin ? "Edit Task" : "Add Technician Note"
                            }
...
const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.title && formData.date) {
      onSave({
        ...(taskToEdit ?? {}), // keep existing id if editing
        ...
        >
          ...
          <div className="pt-2 flex gap-3 pb-4">
            ...
            >
              {taskToEdit ? (
                <>
                  <Save size={18} /> {isAdmin ? "Save Changes" : "Update Note"}
                </>
              ) : (
                "Create Task"
              )}
