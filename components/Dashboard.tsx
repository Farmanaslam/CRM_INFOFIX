...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const metrics = useMemo(() => {
    ...
    const totalCustomersInZone = customers.filter((c) =>
      customerEmailsInZone.has(c.email.toLowerCase())
    ).length;
  ...
  const storeLoads = useMemo(() => {
    ...
    activeTickets.forEach((t) => {
      // fallback in case t.store doesn't exactly match
      const storeName = settings.stores.find(
        (s) => s.name.toLowerCase() === t.store.toLowerCase()
      )?.name;
      if (storeName) loadMap[storeName] = (loadMap[storeName] || 0) + 1;
    });
    ...
    return relevantStores
      .map((s) => ({
        ...
        percent: Math.round(((loadMap[s.name] || 0) / totalActive) * 100),
      }))
  ...
  const technicianLoads = useMemo(() => {
    ...
    activeTickets.forEach((t) => {
      if (t.assignedToId)
        loadMap[t.assignedToId] = (loadMap[t.assignedToId] || 0) + 1;
    });
    ...
    return relevantTechs
      .map((tech) => ({
        ...
        percent: Math.round(((loadMap[tech.id] || 0) / totalActive) * 100),
      }))
      .sort((a, b) => b.count - a.count);
  }, [zoneFilteredTickets, settings.teamMembers, selectedZoneId]);
